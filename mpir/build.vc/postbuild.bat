...
if /i "%filename%" EQU "mpirxxd" goto skip 
...
if "%4" EQU "dll" (
	...
	copy %1mpird.dll %2mpird.dll > nul 2>&1
	copy %1mpird.dll %2mpird.dll > nul 2>&1
	copy %1mpird.exp %2mpird.exp > nul 2>&1
	copy %1mpird.lib %2mpird.lib > nul 2>&1
	if exist %1mpird.pdb (copy %1mpird.pdb %2mpird.pdb  > nul 2>&1)
) else if "%4" EQU "lib" (
    if "%5" EQU "mpir" ( 
  	    if exist %1mpir.lib (
        copy %1mpir.lib %2mpir.lib > nul 2>&1
    ...
    ) else if "%5" EQU "mpirxxd" (
	    if exist %1mpirxxd.lib (
        copy %1mpirxxd.lib %2mpirxxd.lib > nul 2>&1
	    if exist %1mpirxxd.pdb (copy %1mpirxxd.pdb %2mpirxxd.pdb > nul 2>&1)
        )
    )
) else (
